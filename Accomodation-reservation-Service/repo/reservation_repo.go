...
func (repo *ReservationRepository) GetAllAcceptedReservationsForUser(userId string) (*[]domain.Reservation, error) {
	...
	result := repo.DatabaseConnection.Model(&domain.Reservation{}).
		...
		Where(`reservation_requests.user_id = ? and reservation_requests.status = ?`, userId, domain.Active).
		Find(&reservations)
...
func (repo *ReservationRepository) GetAllAcceptedReservationsForAllAccommodations(userId string) (*[]domain.Reservation, error) {
	...
	result := repo.DatabaseConnection.Model(&domain.Reservation{}).
		Joins("JOIN reservation_requests ON reservations.request_id = reservation_requests.id").
		Where(`reservation_requests.accomodation_id = ? reservation_requests.status = ?`, userId, domain.Active).
		Find(&reservations)
